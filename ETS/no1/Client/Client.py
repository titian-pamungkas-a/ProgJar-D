...
def make_socket(destination_address='localhost',port=16000):
    try:
...
def getdataspemain(threadNumber):
    dataPlayer = dict()
    timeStart = datetime.datetime.now()
    for i in range (threadNumber):
        dataPlayer[i] = threading.Thread(target = getdatapemain, args=(random.randint(1, 20), False))
        dataPlayer[i].start()
    for i in range (threadNumber):
        dataPlayer[i].join()
...
if __name__=='__main__':
    hasil_waktu = getdataspemain(20)
    print("Selisih waktu", hasil_waktu)
